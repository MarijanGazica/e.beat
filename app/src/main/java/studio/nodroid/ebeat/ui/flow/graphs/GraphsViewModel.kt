...
class GraphsViewModel(userRepository: UserRepository, private val readingRepo: PressureDataRepository) : ViewModel() {
    ...
    fun time30selected() {
        scope.launch {
            ...
            readings.value = readingRepo.getAllReadingsFor(selectedUser!!.id).filter { it.timestamp in period.startStamp..period.endStamp }
            events.value = Action.SHOW_GRAPH
        }
    }
    ...
    fun dateSelected(isInitial: Boolean, chosenDate: Date) {
        if (isInitial) {
            ...
            scope.launch {
                readings.value = readingRepo.getAllReadingsFor(selectedUser!!.id).filter { it.timestamp in startDate!!..chosenDate.toTimestampEnd() }
                startDate = null
                events.value = Action.SHOW_GRAPH
            }
        }
